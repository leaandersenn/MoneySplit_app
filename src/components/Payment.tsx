...
const Payment = (props: paymentProps) => {
    ...
    const email = FIREBASE_AUTH.currentUser?.email as string;
    const share = participantsMap.get(email)

    const partOfPayment = participantsMap.has(email)

    
    return (
        (((props.creatorData?.email == email)) ?
        ...
        ((partOfPayment) ? 
            ...
            (<View id={id} style={styles.bluePayment}> 
                ...
                <XSmallTextWhite>{`You are not a part of the payment`}</XSmallTextWhite>
            </View>)
        )
        ) 
  )
